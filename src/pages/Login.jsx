...
export default function SignUpForm() {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim() && currentView === 'signup') {
    ...
    if (currentView === 'signup' && formData.role === 'seller') {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (currentView === 'signup') {
        const roleText = formData.role === 'seller' ? 'Seller' : 'Buyer';
        alert(`Welcome ${formData.name}! Your ${roleText} account has been created successfully.`);
      } else {
        alert(`Welcome back! Logged in successfully as ${formData.role}.`);
      }
      
      // Reset form
      setFormData({ name: '', email: '', password: '', phone: '', role: 'buyer' });
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-100 via-green-50 to-blue-100">
      {/* Header Navigation */}
      <header className="flex flex-col sm:flex-row justify-between items-center px-4 sm:px-8 py-4 sm:py-6 gap-4">
        <nav className="flex flex-wrap justify-center sm:justify-start gap-4 sm:gap-8">
          <a href="#" className="text-gray-700 font-medium hover:text-gray-900 transition-colors text-sm sm:text-base">HOME</a>
          <a href="#" className="text-gray-700 font-medium hover:text-gray-900 transition-colors text-sm sm:text-base">COLLECTION</a>
          <a href="#" className="text-gray-700 font-medium hover:text-gray-900 transition-colors text-sm sm:text-base">ABOUT</a>
          <a href="#" className="text-gray-700 font-medium hover:text-gray-900 transition-colors text-sm sm:text-base">CONTACT</a>
        </nav>
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          ...
          <button className="p-2 hover:bg-white hover:bg-opacity-20 rounded-full transition-colors relative">
            <svg className="w-5 h-5 sm:w-6 sm:h-6 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m.4 2L7 13m0 0l-2.7 2.7M7 13h10M12 21a2 2 0 100-4 2 2 0 000 4z" />
            </svg>
            <span className="absolute -top-1 -right-1 bg-black text-white text-xs rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center">3</span>
          </button>
        </div>
      </header>
      ...
      <div className="flex items-center justify-center px-4 sm:px-8 py-8 sm:py-12 min-h-[80vh]">
        <div className="w-full max-w-sm sm:max-w-md">
          ...
          <div className="space-y-4 sm:space-y-6">
                  ...
                  >
                    <div className="flex items-center justify-center space-x-2">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M8 11v4a1 1 0 001 1h6a1 1 0 001-1v-4M8 11H7a1 1 0 00-1 1v8a1 1 0 001 1h10a1 1 0 001-1v-8a1 1 0 00-1-1h-1" />
                      </svg>
            ...
            {currentView === 'signup' && (
              <div className="text-center">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                  ...
                  {formData.role === 'seller' ? '🏪 Seller Registration' : '🛒 Buyer Registration'}
                </span>
              </div>
            )}
            ...
            {currentView === 'signup' && (
              <div>
                <input
                  type="text"
                  name="name"
                  placeholder={formData.role === 'seller' ? 'Business/Store Name' : 'Full Name'}
                  value={formData.name}
            ...
            {currentView === 'signup' && formData.role === 'seller' && (
              <div>
                <input
                  type="tel"
                  name="phone"
                ...
                <p className="text-xs text-gray-600 mt-1">Required for customer contact and order management</p>
              </div>
            )}
            ...
            >
              {isLoading ? (
                <>
                  ...
                  {currentView === 'signup' ? 'Creating Account...' : 'Logging in...'}
                </>
              ) : (
                <>
                  {currentView === 'signup' ? (
                    formData.role === 'seller' ? '🏪 Create Seller Account' : '🛒 Create Buyer Account'
                  ) : (
                    'Login'
                  )}
                </>
            ...
            {currentView === 'signup' && (
              <div className="mt-6 p-4 bg-white bg-opacity-30 rounded-lg">
                <h3 className="font-medium text-gray-800 mb-2">
                  {formData.role === 'seller' ? 'Seller Benefits:' : 'Buyer Benefits:'}
                </h3>
